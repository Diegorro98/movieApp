...
class MovieGenreViewModel (private val repository: DatabaseRepository): ViewModel() {
    ...
    fun getMovies(genreID: Int): LiveData<List<MovieGenre>> = repository.getGenreMovies(genreID).asLiveData()
    fun getGenres(movieID: Int): LiveData<List<MovieGenre>> = repository.getMovieGenres(movieID).asLiveData()
}
